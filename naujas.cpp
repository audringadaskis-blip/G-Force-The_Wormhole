...
int main() {
    ...
    if(!texture.loadFromFile("assets/character.png")) {
        std::cout << "could not load texture1";
        return 0;
    }
    ...
    //characterSprite.setPosition(player.getPosition());
    sf::Vector2f playerPos = {100, 220};
    ...
    while (window.isOpen()) {
        ...
        sf::Vector2f spriteOffset = {0.f, -50.f};
        characterSprite.setPosition(playerPos + spriteOffset);
